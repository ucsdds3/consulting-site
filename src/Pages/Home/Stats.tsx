...
const Stats = () => {
  ...
  const startCounting = () => {
    const interval = setInterval(() => {
      setStatsData((prev) =>
        prev.map((num, index) => Math.min(num + maxNumbers[index] / 15, maxNumbers[index]))
      );
    }, 50);
  ...
  return (
    <div className="flex flex-col items-center gap-10 justify-start text-white py-24 h-screen w-screen">
      <motion.h1
